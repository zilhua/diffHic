...
<<>>=
keep <- filterPeaks(data, enrichments, min.enrich=0.5, min.count=5, min.diag=2L)
sum(keep)
@
...
#keep2 <- keep & !is.na(dist)
#which(keep2)[order(enrichments[keep2], decreasing=TRUE)][1:100]
#plotPlaid(input[3], param=mm.param, width=2e5, cap=20, col="black",
...
for (chosen in 1:length(a.list)) { 
	if (!suppressWarnings(any(overlapsAny(anchors(data[keep]), a.list[chosen], type="equal") & 
			overlapsAny(targets(data[keep]), t.list[chosen], type="equal")))) { stop("can't find example for peak-calling") }
	plotPlaid(input[3], param=mm.param, width=2e5, max.count=capped[chosen], col="black",
...
boxed <- boxPairs(data[keep], reference=neighbourhood)
out <- tabulate(tabulate(boxed$indices[[1]]))
setNames(round(c(out[1:5], sum(out[5:length(out)]))/sum(out)*100, 1), c(1:5, ">5"))
keep2 <- filterPeaks(data, enrichments, min.enrich=0, min.count=5, min.diag=2L)
boxed <- boxPairs(data[keep2], reference=neighbourhood)
out <- tabulate(tabulate(boxed$indices[[1]]))
setNames(round(c(out[1:5], sum(out[5:length(out)]))/sum(out)*100, 1), c(1:5, ">5"))
@
...
In such cases, it may be more practical to use the \code{neighbourCounts} function.
...
<<>>=
en.data <- neighbourCounts(input, param, width=1e5, filter=20, flank=5)
en.data$interaction
