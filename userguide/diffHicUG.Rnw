...
While count sizes will also decrease, this is not a major problem as counts should be large at low interaction distances. 
...
This is mostly for consistency, so that all operations are performed on \code{data} in this guide.
