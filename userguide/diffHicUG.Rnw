...
%Finally, one can consider interesting events as peaks in read pair density in the two-dimensional interaction space.
...
%The filter threshold for each bin pair is defined from the average count across all the neighbouring bin pairs in the interaction space. 
...
%
%<<>>=
%surrounds <- countNeighbors(data, flank=2)
%surrounds$y
%@
%
%The abundance for each bin pair is calculated and compared to the abundance of its neighbourhood.
%This is done using the \code{aveMb2} function, which accounts for the relative differences in read counting area.
%Values of \code{n} indicate the number of bin pairs constituting each neighbourhood.
%
%<<>>=
%neighbor.ab <- aveMb2(surrounds$y, areas=surrounds$n, relative=TRUE)
%enrichment <- aveMb2(data, areas=1, relative=TRUE) - neighbor.ab
%summary(enrichment)
%@
%
...
%Some care is required in dealing with bin pairs that have no neighbourhood, i.e., \code{n=0}, as these will have \code{NA} values for the enrichment.
%
%<<>>=
%local.keep <- count.keep & enrichment > log2(2) & !is.na(enrichment)
%summary(local.keep)
%@
%
%% I've refrained from adding this to the UG, because the results are, at best, mixed.
%% It seems to be difficult to call peaks in 2D space because of the variability in peak shapes, which makes it hard to define an appropriate background region.
%% The diagonal background works well in theory, but in practice, it tends to pick up vertical striations in the interaction space.
%% It also picks up junctions between compartments and edges of large domains, which are hardly the punctate peaks that we're looking for.
%% So, the diagonal shape computes background for the wrong things, but I can't think of any cleaner way to define the local background for intra-chromosomals.
%% Indeed, any other background shape would fall victim to these things as well, because inclusion of the low-density area would dilute the neighbourhood counts.
%% You could look around the site and take the maximum abundance of the neighbouring bin pairs.
%% However, I'd suspect that would be too conservative due to edge effects, e.g., if your central site doesn't capture all of the peak.
%% So, I think that this entire strategy is a bust; there's too much uncertainty about what we should be looking for.
%% Inter-chromosomals aren't so bad, but that's hardly the problem.
...
%As the total number of bin pairs increases when the bin size decreases, increasing the stringency of the threshold effectively serves as a multiplicity correction.
...
For example, calculation of the median or fitting of the trend will be complicated by discreteness in the abundances.
This is avoided with larger bin sizes.
...
In all cases, adjustment of abundances by area fails to account for other differences between bins, e.g., in mappability or sequenceability.
Full consideration of these bin-specific biases requires more complex methods such as iterative correction (see Section~\ref{sec:itercor}).
...
Filtering can also be performed to select for peaks in the interaction space.
This refers to bin pairs with substantially more reads than its neighbours.
...
Note that the function requires data without any filtering.
...
enrich.chr1 <- enrichedGap(chr1.data, width=new.bin.size)
summary(enrich.chr1)   
@
...
% We use a different definition from Rao, in that we ignore the donut and we
% instead test each quadrant.  I think that this provides better protection
% against compartments, where it'll be the top-right or bottom-left/right which
% has the high-intensity structure. In theory, the vertical/horizontal stripes
% also protect against compartments, but that depends on those compartments
...
The example below retains those bin pairs with average abundances and enrichment values above 1.
Both thresholds are arbitrarily chosen as they ultimately depend on what the user considers to be interesting.
...
for (chosen in 1:length(a.list)) { 
	...
	plotPlaid(input[3], param=mm.param, width=2.5e4, cap=capped[chosen], col="black",
...
The \code{bin.size} and \code{mm.frag} objects are required from Chapter~\ref{chap:counting}.
