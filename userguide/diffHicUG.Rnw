...
Counts for each chromosome pair can then be reloaded for filtering, and the remaining pairs aggregated for further analysis.
This strategy is effective but slow.
