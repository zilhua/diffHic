...
\begin{center}
<<echo=FALSE,fig=TRUE>>=
redist <- getDistance(data) # Prettifying the plot by removing near-diagonal artifacts.
temp <- data
dist.keep <- is.na(redist) | redist > 1.5e6
data <- data[dist.keep]
<<maunnorm>>
<<makema>>
data <- temp
...
\begin{center}
<<echo=FALSE,fig=TRUE>>=
temp <- data # Showing the fitted results after removing diagonal artifacts.
data <- data[dist.keep]
temp.off <- nb.off
nb.off <- normalize(data, type="loess")
<<manorm>>
