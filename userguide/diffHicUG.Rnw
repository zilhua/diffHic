...
<<>>=
new.param <- reform(mm.param, restrict=rbind("chr2", "chr19"))
new.param
@
...
Filtering can be performed separately for each chromosome pair.
This is effective for local filtering, but less so for direct or trended filtering where information from multiple pairs is required for threshold calculation.
