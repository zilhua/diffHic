...
For simplicity, counting will be performed with the default settings in this guide, i.e., no values for \code{restrict} or \code{discard}.
Users are encouraged to experiment for their own analyses.
However, the same \code{pairParam} object should be used between the different functions in the \pkgname{} pipeline.
