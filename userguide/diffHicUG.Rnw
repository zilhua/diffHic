...
<<>>=
y <- asDGEList(data)
y$offset <- normalizeCounts(counts(data), totals(data), type="loess")
@
...
The \code{prunePairs} function removes read pairs corresponding to artifacts in a specified index file.
The returned vector contains the number of read pairs that were removed for each of the various reasons described below.
The number of read pairs remaining is also recorded.
All retained read pairs are then placed in an output index file for downstream use.
...
For full compatability, users should include the alignment positions and lengths as \code{xxx.pos} and \code{xxx.len} (replacing \code{xxx} with \code{anchor} or \code{target}) in \code{dummy}.
...
This is more useful for other \pkgname{} functions than some arbitrary \code{other.data}.
...
Also note that the boundary of each bin is rounded to the closest restriction site in \code{squareCounts}.
...
For example, the code below takes the repeats called by the RepeatMasker program in the hg19 build of the human genome.
...
For simplicity, counting will be performed with the default settings, i.e., no values for \code{restrict} or \code{discard}.
Generally, the same \code{pairParam} object should be used between the different counting functions described in this chapter.
...
<<>>=
require(csaw)
nb.off <- normalizeCounts(counts(data), totals(data), type="loess")
head(nb.off)
@
...
y.small$offset <- normalizeCounts(counts(smaller.data), totals(smaller.data), type="loess")
...
<<>>=
matched <- boxPairs(reference=bin.size, smaller=smaller.data, mm.param)
@
...
<<>>=
matched <- boxPairs(bin.size, larger=data, smaller=smaller.data, mm.param)
