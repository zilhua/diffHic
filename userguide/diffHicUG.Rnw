...
Note that the larger bin size must be a multiple of the \code{width} used for the smaller bins.
...
Misinterpretation is mitigated by nesting the smaller bin pairs within their larger counterparts.
