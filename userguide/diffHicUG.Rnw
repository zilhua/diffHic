...
<<>>=
clustered <- clusterPairs(smaller.data, tol=1, max.width=1e6)
head(clustered)
...
tab.all <- data.frame(result.small$table, PValue=tabcluster$PValue, FDR=tabcluster$FDR)
write.table(tab.all[o,], file="clustered.tsv", sep="\t", quote=FALSE, row.names=FALSE)
head(tab.all)
@
