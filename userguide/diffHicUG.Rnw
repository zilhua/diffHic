...
Here, the threshold is defined as a minimum fold change of 3 above this estimate.
...
The \code{filterPeaks} function computes an enrichment value for each bin pair, defined as the log-fold change of its abundance against its neighbours.
...
The \code{scaledAverage} function from \csaw{} is applied internally to compute the average abundance for each of the 1 Mbp bin pairs. 
...
Filtering can then be performed based on a minimum fold change over this threshold (in this case, 10-fold).
...
direct$threshold
small.keep <- smaller.ab > direct$threshold + log2(10)
