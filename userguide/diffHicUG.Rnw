...
DNase-based Hi-C is a variant of the standard technique, whereby the DNase~I enzyme is used to cleave the genome instead of restriction enzymes.
This allows for resolution beyond that of individual restriction fragments.
However, cleavage sites for DNase cannot be easily predicted, such that \code{param\$fragments} is no longer sensibly defined.
The formation of index files is also inappropriate, as there are no restriction fragments for reads to be assigned to.
...
flank.width <- 5
enrich.chr1 <- enrichedPairs(chr1.data, flank=flank.width)
summary(enrich.chr1)   
@

More specifically, the function partitions the neighbourhood into several different regions around the target bin pair.
The size of the neighbourhood is set by \code{flank}, and is defined in terms of bins (actual distance of $\sim$500 kbp, in this case).
...
Larger numbers suggest that more aggressive filtering on the enrichment score is required, e.g., in the second example. 

<<>>=
neighbourhood <- (2*flank.width + 1) * exptData(chr1.data)$width
boxed <- boxPairs(chr1.data[keep], reference=neighbourhood)
table(tabulate(boxed$indices[[1]]))
keep2 <- filterPeaks(chr1.data, enrich.chr1, min.enrich=0, min.count=5, min.diag=2L)
boxed <- boxPairs(chr1.data[keep2], reference=neighbourhood)
out <- tabulate(tabulate(boxed$indices[[1]]))
setNames(c(out[1:10], sum(out[10:length(out)])), c(1:10, ">10"))
@
