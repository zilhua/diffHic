...
enrich.chr1 <- enrichedPairs(chr1.data, flank=5)
summary(enrich.chr1)   
@

More specifically, the function partitions the neighbourhood into several different regions around the target bin pair.
The size of the neighbourhood is set by \code{flank}, and is defined in terms of bins (actual distance of 500 kbp, in this case).
...
The neighbourhood regions are designed to capture high-intensity structural features like TADs or banding patterns \citep{rao2014kilobase}.
Any bin pair inside a feature will be compared to other high-abundance bin pairs from the same feature.
This avoids spuriously high enrichments if a bin pair within a feature was compared to those outside the feature.
...
<<>>=
keep <- filterPeaks(chr1.data, enrich.chr1, min.enrich=0.5, min.count=5, min.diag=2L)
sum(keep)
@
...
The density of bin pairs around the retained peaks is another useful diagnostic.
Retained bin pairs are nested within larger ``parent'' bin pairs, using the \code{boxPairs} function.
The density can then be computed as the proportion of parent's interaction space that is covered by the retained bin pairs.
High densities suggest that whole structural features are being retained, which means that a larger enrichment threshold is required.
In this case, most parents have low densities ($< 10$\%), indicating that the chosen threshold is generally appropriate.

<<>>=
feature.width <- 5e5 # Expected width of TADs.
boxed <- boxPairs(chr1.data[keep], reference=feature.width)
area.per.bin <- getArea(chr1.data[keep])
densities <- by(area.per.bin, boxed$indices[[1]], FUN=sum)/getArea(boxed$pairs)
c(summary(densities), "Above 10%"=sum(densities > 0.1)/length(densities))
@
...
tabcluster <- csaw::combineTests(clustered$id, result.small$table)
head(tabcluster)
@
...
final <- data.frame(anchor.chr=seqnames(ax.c), anchor.start=start(ax.c), 
...
However, users should always keep in mind that the FDR is computed with respect to features, not bin pairs.

<<>>=
small.ids <- cons$id$smaller
inside <- csaw::getBestTest(small.ids, result.small$table)
ax.3 <- as.data.frame(anchors(smaller.data))[inside$best,]
tx.3 <- as.data.frame(targets(smaller.data))[inside$best,]
...
head(nested)
expanded <- rep(NA, nrow(final.2)) # As some parents do not have nested elements.
expanded[as.integer(rownames(inside))] <- 1:nrow(inside)
final.3 <- data.frame(final.2, nest=nested[expanded,])
