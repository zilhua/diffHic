...
require(edgeR)
keep <- aveLogCPM(counts(data), lib.size=totals(data)) > 0
data <- data[keep,]
y <- DGEList(counts(data), lib.size=totals(data))
require(csaw)
y$offset <- normalizeChIP(counts(data), lib.sizes=totals(data), type="loess")
...
Again, anchor and target notation applies where the interval with the larger start coordinate in the genome is defined as the anchor.
...
Resorting is also performed though the indices of the original regions is supplied in the metadata as \code{original}.
...
adjc <- cpm(counts(margin.data), lib.size=totals(margin.data), log=TRUE, prior.count=5)
...
One can check that this is the case by ensuring that the regions and totals are the same.

<<>>=
identical(regions(data), regions(margin.data))
identical(totals(data), totals(margin.data))
@

Also note that, technically speaking, the anchor and target notation here is superfluous.
...
require(edgeR)
prior.count <- 5
ave.ab <- aveLogCPM(counts(data), lib.size=totals(data), prior.count=prior.count)
...
<<>>=
small.ab <- aveLogCPM(counts(smaller.data), lib.size=totals(smaller.data),
...
The same procedure can be used in direct filtering by replacing \code{trend.threshold[...]} with \code{direct.threshold} when computing \code{new.threshold}.
...
<<>>=
re.ab <- aveLogCPM(counts(redata), lib.size=totals(redata)) 
...
o <- order(ab)
adj.counts <- cpm(counts(data), lib.size=totals(data), log=TRUE)
<<makema>>
@
...
<<>>=
require(csaw)
nb.off <- normalizeChIP(counts(data), lib.size=totals(data), type="loess")
head(nb.off)
@
...
As one might imagine, copy number variations (CNVs) in the interacting regions will affect the interaction intensity. 
These CNV-driven differences in intensities are generally uninteresting and must be removed to avoid spurious detection.
This is done using the \code{normalizeCNV} function, which simultaneously removes the trended biases described earlier.
...
For any two libraries, the ratio of marginal counts for a bin can be used as a proxy for the relative CNV between libraries in that bin.
Each bin pair is subsequently associated with two marginal log-ratios.
...
Any systematic differences between libraries is attributable to CNV (or trended) biases and is removed.
...
y.small$offset <- normalizeChIP(counts(smaller.data), lib.sizes=totals(smaller.data), type="loess")
...
Of course, one could just count read pairs using larger bin pairs in the first place.
This provides larger counts and improves power for detecting differences, given that spatial resolution is not an issue for diffuse interactions.
Thus, the method described above is not particularly useful.
Nonetheless, it is a good introduction to the method in the next section (which is more useful).

\subsection{Merging results from different bin widths}
\label{sec:mergebins}
The choice of bin size is not clear when there might be sharp and diffuse changes in the interaction space.
