...
All clusters formed at the chosen \code{threshold} can then be reported, as shown previously.
...
<<>>=
ax.s <- anchors(cons$pairs)
tx.s <- targets(cons$pairs)
o.s <- order(cons$table$PValue)
results.s <- data.frame(anchor.chr=seqnames(ax.s), anchor.start=start(ax.s), 
    ...
    target.end=end(tx.s), cons$table)
...
The \code{domainDirections} function represents sites with 100 kbp bins, and counts the number of read pairs mapped between each bin and the closest \code{span} up- or downstream sites.
The genomic coordinates of each bin is returned along with the total count in either direction, averaged across \code{input} libraries.
...
This seems to be easier to model with a Gaussian distribution, as shown for chromosome 1.
A large prior count is used to avoid spuriously large values at low counts.
...
Actual identification of the domains involves fitting a hidden Markov model (HMM) to the directionalities across each chromosome.
Three states are used -- upstream bias, no bias and downstream bias, corresponding to the start, middle and end of a domain with positive, near-zero and negative directionalities.
...
Note that the normality of the log-fold changes allows easy modelling with a single Gaussian distribution, instead the mixture of Gaussians used by \citeauthor{dixon2012topological}
