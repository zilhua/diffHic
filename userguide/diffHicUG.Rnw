...
Chimera alignment can be achieved with approaches like iterative mapping \citep{imakaev2012iterative} or read splitting \citep{seitan2013cohesin}.
A mapping script is provided in \pkgname{} that is based on the latter.
This defines the ligation signature as the sequence that is obtained after ligation between two blunt ends derived from cleaved restriction sites.
For example, the \textit{Hin}dIII enzyme cleaves at \texttt{AAGCTT} with a 4 bp overhang.
This yields a ligation signature of \texttt{AAGCTAGCTT} in chimeric reads.
The ligation signature in each read is identified with cutadapt \citep{martin2011cutadapt}, and the read is split into two segments at the center of the signature.
Each segment is then aligned using Bowtie2 \citep{langmead2012bowtie}.
...
The gap distance is defined as the linear distance between two paired reads on the same chromosome.
The \code{min.ingap} paramater removes inward-facing intra-chromosomal read pairs where the gao distance is less than the specified value.
...
This can be repeated for intra-chromosomal read pairs, by plotting the distribution of gap distances for each strand orientation \citep{jin2013highres}.
...
Each pairwise interaction is parameterized by two genomic intervals representing the interacting loci.
...
Other pre-specified regions can be used, e.g., known enhancers or protein binding sites.
...
To avoid any confusion in the notation, the count for each bin pair is defined as the interaction count.
It may be necessary later to process the interaction count based on the marginal counts for the corresponding bins in the pair.
In such cases, one should ensure that the same \code{width} (and other parameters) is used in \code{squareCounts} and \code{marginCounts}.
...
Also note that, technically speaking, the anchor and target notation is superfluous for \code{marginCounts}.
This is because the counts refer to bins on the linear genome, rather than pairs of bins in the interaction space.
Nonetheless, a \code{DIList} is still returned by \code{marginCounts} for convenience and consistency.
Both anchor and target indices will be identical in this object.
...
Low counts will also interfere with the asymptotic approximations used later in the analysis.
The example below removes those bin pairs with an overall NB mean below 5 across all libraries.

<<>>=
count.keep <- ave.ab >= aveLogCPM(5, mean(totals(data)), prior.count=prior.count)
summary(count.keep)
...
Note that the result of filtering here are assigned into the \code{dummy} object.
This avoids overwriting the original data at this point in the guide.
Similarly, \code{dummy} will be used as a dumping group for the variuos demonstrations below.
However, when it comes to actual filtering, the filtered results will be assigned back into the \code{data} object for further analysis.
...
However, the interpretation of the filter is not obvious.
...
A non-unity value of \code{filter} is set to avoid using too much memory.

<<>>=
new.bin.size <- 1e5
smaller.data <- squareCounts(input, fragments=mm.frag, width=new.bin.size, filter=20)
@
...
Finally, filtering is performed by mandating a minimum fold change for the abundances of the smaller bin pairs over the adjusted threshold.
...
<<>>=
nspec.dist <- ave.ab[is.na(dist)]
sum(nspec.dist > direct.threshold+log2(10))
sum(nspec.dist > direct.threshold+log2(100))
@
...
The same procedure can be used in trended filtering by replacing \code{direct.threshold} with \code{trended.threshold}.
...
If artifact generation and removal is not consistent between libraries, the behaviour of the diagonal elements will differ markedly from the other bin pairs.
This can be diagnosed using MA plots, where the diagonal elements show up as a distinct mass that is unconnected to the rest of the points.
Such irregularities cannot be smoothly normalized and should be removed prior to further analysis, or at least analyzed separately.
...
Users are advised to switch to a smaller bin size to recover these interactions.
With smaller bins, artifacts will still be restricted to the diagonal (given bin sizes above 25 kbp)  but off-diagonal bin pairs can now be retained.
This is also easier to interpret, given that the spatial resolution is greater than that for large bins.
Count size is usually not an issue for these smaller bins, given that counts are high for short-range interactions.
...
Changes in cross-linking efficiency or ligation specificity can lead to a redistribution of read pairs throughout the interaction space. 
...
Here, the \code{normalizeCounts} function in the \csaw{} package is used on the bin pair counts.
This implements the NB-loess method, which is an adaptation of existing non-linear methods for discrete count data \citep{normpaper}.
The function computes an offset term for each bin pair in each library for use in a generalized linear model (GLM). 
...
Any systematic differences are assumed to be technical in origin and are subsequently removed.
If this assumption does not hold, removal of the trend may result in the loss of some genuine differences between conditions.
...
The latter effect would result in a trended bias, but iterative correction would have no effect due to the former.
...
For any two libraries, the ratio of marginal counts for each bin can be used as a proxy for the relative CNV between libraries in that bin.
This means that each bin pair will be associated with two of these marginal log-ratios.
Note that the marginal counts should be collected with the same parameters as the bin pair (i.e., interaction) counts.
...
The response for each bin pair is defined as the log-ratio of interaction counts between two libraries.
A locally weighted surface is fitted to the response against all three covariates.
The idea is that, at any combination of covariate values, most bin pairs are not expected to be differential.
Any systematic differences between libraries is attributed to CNV-driven (or trended) biases and is removed.
The function does so by returning GLM offsets, which can be supplied to the statistical analysis to eliminate the bias.

The use of a empirical trend fit reduces the number of assumptions involved in translating a CNV into its effect on interaction intensities.
For example, iterative correction can also be used to correct for CNVs by accounting for differences in coverage of each bin.
It does so by assuming factorizability \citep{imakaev2012iterative}, i.e., the effect on intensity is a product of the CNVs of the interacting regions.
This may not be reliable given that random ligation is not the only mechanism for read pair generation.

That said, the use of many covariates may lead to overfitting and removal of genuine differences.
The function also assumes that CNVs in different parts of the genome have the same effect.
Caution is therefore required when using \code{normalizeCNV}. 
The safest choice is to avoid it when CNVs are not present, according to the diagnostic plots in Section~\ref{sec:marginal}.

\subsection{Visualizing the effect of CNV removal}
Here, the \citeauthor{rickman2012oncogene} dataset is used as it contains more CNVs.
...
rick.data <- squareCounts(count.files, hs.frag, width=1e6)
rick.marg <- marginCounts(count.files, hs.frag, width=1e6)
rick.data <- rick.data[aveLogCPM(asDGEList(rick.data)) > 0,]
@

The aim is to plot the log-ratio between the two libraries for the interaction counts, as a function of the marginal log-ratios.
This will illustrate the effect of the CNV on the interaction intensities between libraries.
...
amatch <- match(anchors(rick.data, id=TRUE), anchors(rick.marg, id=TRUE))
tmatch <- match(targets(rick.data, id=TRUE), anchors(rick.marg, id=TRUE))
sum.madjc <- m.adjc[amatch,] + m.adjc[tmatch,]
mlr <- sum.madjc[,1] - sum.madjc[,2]
@

Plotting can then be performed to determine the effect of \code{normalizeCNV}.
...
Note that \code{maxk} is necessary to increase the allocated memory in the call to \code{locfit} during normalization.
...
smoothScatter(mlr, before[,1]-before[,2], ylim=c(-4, 4), main="Before",
    xlab="Sum of marginal log-ratios", ylab="Interaction log-ratio")
smoothScatter(mlr, after[,1]-after[,2], ylim=c(-4, 4), main="After",
    xlab="Sum of marginal log-ratios", ylab="Interaction log-ratio")
@
...
This can be incorporated into the testing framework to (correctly) reduce the significance of detected interactions when the data is highly variable.
...
This should manifest as a smooth trend in the NB dispersion estimates with respect to the average abundances.
...
This stabilizes the dispersion estimates and improves precision/power for downstream applications.
...
It is important to use the \code{robust=TRUE} argument in \code{glmQLFTest}.
...
Use of smaller bin pairs can lead to problems when diffuse interactions are present.
Recall that the FDR refers to the proportion of bin pairs that are false positives.
As the bin pair is just an analytical construct, and the actual statistic of interest is the equivalent proportion for the underlying interactions.
...
This is reasonable for pairs of large bins, but is less so when multiple smaller bin pairs are used to represent a diffuse interaction.
...
One can then combine $p$-values across all bin pairs in the cluster, to obtain a single combined $p$-value for the cluster.
...
The resulting combined $p$-value represents the evidence against the global null hypothesis for the larger bin pair.
That is, there are no significant differences in any of the smaller bin pairs nested within the larger bin pair.
...
The use of small bin pairs will allow detection of the former, while bin clustering will prevent the results being dominated by the latter.
...
All of these $p$-values can be combined using Simes' method, similar to that previously described.
The idea is to provide a compromise between larger counts and spatial resolution in the final combined $p$-value.

Some finesse is necessary to assign different weights to each $p$-value during the combining procedure \citep{benjamini1997multiple}.
For each larger bin pair, the $p$-value for that bin pair has the same weight as those of all the smaller nested bin pairs.
This means that the analysis with the larger bin pair has the same contribution as that for the smaller bin pairs.
...
This effectively means that the $p$-values from each bin size are assigned equal weight.
Calculation of the combined $p$-value is then performed using the \code{combineTests} function in the \csaw{} package.
...
However, decreases are also possible as the effective number of tests increases the severity of the correction.
