...
Each row of the returned \code{DIList} contains the read pair counts for a single genomic bin.
...
This is trivially transformed into the average abundance, which can be computed using the \code{aveLogCPM} function in \edgeR{} \citep{mccarthy2012glm}.
...
As an aside, the filtered results are assigned into the \code{dummy} object.
...
This is obtained using the \code{getDistance} function, which computes the distance between the midpoints of the bins in each bin pair.
...
% Technically, we should include the empty bin pairs that we don't count.
% However, the direct.threshold here passes through the mode of the histogram of abundances anyway, so it looks pretty good.
% It doesn't seem that adding those guys would help.
% You'd only get a benefit if a lot of them were zeros (e.g., for smaller bins); but then, the median would be imprecise if the counts were so low.
% Similarly, for the trended filter, zeros would get ignored by the robustness routines.

The \code{direct.keep} vector can then be used to filter \code{data}, as shown above.
...
In practice, the direct filter can be combined with \code{count.keep} to ensure that the retained bin pairs also have large absolute counts.
...
For example, a two-fold increase above the expected compaction intensity may be necessary.
...
Note that the function requires unfiltered bin pairs, i.e., counting in \code{squareCounts} with \code{filter=1}.
...
The \code{scaledAverage} function from the \csaw{} package is applied to recompute the average abundance for each of the 1 Mbp bin pairs. 
...
The resulting values are used to obtain a direct filter threshold, as described.
...
The example below uses a non-linear method from \csaw{}, adapted from existing methods to handle discrete count data. %\citep{normpaper}. % Coming soon, I hope.
An offset term is computed for each bin pair in each library, to use in a generalized linear model (GLM). 
A large offset for an observation is equivalent to downscaling that count relative to the counts of the other libraries. 
...
Most of the trend is removed, indicating that normalization was successful.
...
%\subsection{Aggregating results for small bin pairs}
%The code below demonstrates the potential for loss of FDR control in real data.
%Significant bin pairs that are adjacent in the interaction space are clustered together.
%Each cluster can be treated as a representative of a single underlying interaction.
%A substantial proportion of clusters consist of multiple bin pairs, which suggests that controlling the FDR across bin pairs may not be equivalent to controlling that across clusters/interactions. 
%
%<<>>=
%small.de <- p.adjust(result.small$table$PValue, method="BH") <= 0.05
%clustered <- clusterPairs(smaller.data[small.de], tol=1)
%table(tabulate(clustered))[1:10]
%@
%
%One solution might be to cluster bin pairs in the interaction space to identify the underlying true interaction.
...
Comprehensive detection of differential interactions can be achieved by combining the differential analyses from both small and large bin pairs.
...
To consolidate these results, all smaller bin pairs that are nested within each of the larger bin pairs are identified using the \code{boxPairs} function.
...
Note that the larger bin size \textit{must} be a multiple of the smaller bin size, to simplify later interpretation.

<<>>=
matched <- boxPairs(larger=data, smaller=smaller.data, 
    param=mm.param, reference=bin.size)
...
In addition, nesting protects against misinterpretation of the FDR for diffuse differential interactions.
...
Misinterpreting one FDR as the other may result in loss of control \citep{lun2014denovo}.
Nesting ensures that the FDR is computed over large bin pairs, even when detection is driven by small bin pairs.

\subsection{Reporting nested bin pairs}
It is often convenient to identify the top-ranked (smaller) nested bin pair within each of the (larger) parent bin pairs.
