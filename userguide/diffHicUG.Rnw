...
The \code{enrichedPairs} function computes an enrichment value for each bin pair, defined as the log-fold change of its abundance against its neighbours.
...
enrich.chr1 <- enrichedPairs(chr1.data)
summary(enrich.chr1)   
@
...
The neighbourhood regions are designed to capture high-intensity structural features like TADs, compartments or banding patterns \citep{rao2014kilobase}.
Any bin pair lying within one of these features will be compared to other high-abundance bin pairs from the same feature.
This avoids spuriously high enrichments when a bin pair within a feature is compared to those outside the feature.

In this particular case, an enrichment value of 1 is required for retention.
This means that the average counts for each retained bin pair are at least two-fold higher than those of its neighbourhood.
...
<<>>=
keep <- filterPeaks(chr1.data, enrich.chr1, min.enrich=1, min.count=5, min.diag=2L)
sum(keep)
@
...
For small bin sizes, adjacent bin pairs can be aggregated into larger clusters.
This avoids redundancy in the interpretation of the results.
...
Misinterpretation of the FDR is also avoided as each interaction is represented by one cluster with one combined $p$-value (see Section~\ref{sec:mergebins}).

<<>>=
clustered <- clusterPairs(smaller.data, tol=1, upper=1e6)
head(clustered)
tabcluster <- csaw::combineTests(clustered, result.small$table)
head(tabcluster)
@
...
Some protection is provided by setting \code{max.width} to limit the maximum dimensions of each cluster.
...
Reporting the cluster is also non-trivial, as there is no guarantee that it will form a regular shape in the interaction space.
One solution is to report bin pairs as a clustered set in the result table.
This ensures that each cluster can be easily identified while preserving all information about the cluster composition.
Here, results are also ranked so the most significant clusters are reported first.
Bin pairs within each cluster are also sorted.

<<>>=
ax <- anchors(smaller.data)
tx <- targets(smaller.data)
o <- order(tabcluster$PValue[clustered], clustered, result.small$table$PValue)
final <- data.frame(clusterId=clustered, anchor.chr=seqnames(ax), anchor.start=start(ax), 
    anchor.end=end(ax), target.chr=seqnames(tx), target.start=start(tx), 
    target.end=end(tx), result.small$table, PValue.cluster=tabcluster$PValue[clustered], 
    FDR.cluster=tabcluster$FDR[clustered])[o,]
write.table(final, file="clustered.tsv", sep="\t", quote=FALSE, row.names=FALSE)
@
...
To combine the results, the \code{consolidatePairs} function is used to identify all smaller bin pairs that are nested within each of the larger ``parent'' bin pairs.
