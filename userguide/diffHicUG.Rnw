...
This is also called the average abundance and can be computed using the \code{average} function \citep{mccarthy2012glm}.
...
Here, the NB-loess method is used to account for the discrete nature of the counts \citep{normpaper}, based on the \code{normalizeCounts} function in the \csaw{} package.
