...
The \code{aveMb2} function can then be applied to compute the average abundance-per-megabase squared for each bin pair. 
...
For consistency, the average abundances of the larger bin pairs are also recomputed with \code{aveMb2}.
...
<<>>=
smaller.ab <- aveMb2(smaller.data, areas=new.bin.size^2)
summary(smaller.ab)
larger.ab <- aveMb2(data, areas=bin.size^2)
...
<<>>=
small.keep <- smaller.ab > new.threshold + log2(100)
small.original <- smaller.data
...
<<>>=
new.log.dist <- log10(getDistance(small.original) + new.bin.size/2)
...
In such cases, \code{aveMb2} can be used to compute the filter statistic.
Note that this differs from the previous application of \code{aveMb2} in Section~\ref{sec:filtersmall}.
There, the adjustment by area is only performed between bin pairs of different sizes.
Relative abundances within each set of bin pairs (i.e., of the same size) are not altered as \code{areas} is set to a constant scalar in each \code{aveMb2} call.
Here, the area is computed separately and will probably be different for each region pair.

<<>>=
summary(aveMb2(redata))
@
