...
He also has a chance of being cited when \edgeR{} is involved.
...
As an aside, it may be convenient to identify the smaller bin pair with the lowest $p$-value that is nested within each of the larger bin pairs.
This means that high-resolution changes within a larger bin pair can be easily identified.
However, users should always keep in mind that the FDR is computed with respect to the larger bin pairs.

<<>>=
inside <- getBestTest(sdex, resul.small$table)
best.vec <- rep(NA, nrow(matched$pairs))
best.vec[as.integer(rownames(inside))] <- inside$best
head(best.vec)
ax.3 <- anchors(smaller.data)[best.vec]
tx.3 <- targets(smaller.data)[best.vec]
final.2 <- cbind(final.2, nest.anchor.chr=seqnames(ax.3), nest.anchor.start=start(ax.3), 
    nest.anchor.end=end(ax.3), nest.target.chr=seqnames(tx.3), 
    nest.target.start=start(tx.3), nest.target.end=end(tx.3),
@
...
plotPlaid(input[1], anchor=expanded.a, target=expanded.t, cap=cap1, 
    width=5e4, col="red", fragments=mm.frag)
