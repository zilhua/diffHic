...
The latter defines the ligation signature as the sequence that is obtained after ligation between two blunt ends derived from cleaved restriction sites.
...
Mapping by read splitting can be performed with a custom Python script in \pkgname{}.

<<results=hide>>=
system.file("python", "presplit_map.py", package="diffHic", mustWork=TRUE)
@

Users are strongly recommended to synchronize mate pair information and to mark duplicate read pairs from the resulting BAM file.
...
The \code{min.ingap} paramater removes inward-facing intra-chromosomal read pairs where the gap distance is less than the specified value.
...
<<>>=
prepped <- preparePairs("SRR027958.bam", hs.frag, file="SRR027958.h5", 
    dedup=TRUE, minq=10)
...
The boundary of each bin is also rounded to the closest restriction site in \code{squareCounts}.
...
In such cases, one should ensure that the same values of \code{width} and \code{fragments} are used in \code{squareCounts} and \code{marginCounts}.
...
This is because the marginal counts refer to bins on the linear genome, rather than pairs of bins in the interaction space.
