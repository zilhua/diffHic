...
The ligated DNA is sonicated and the fragments of DNA containing ligation junctions are purified by a streptavidin pulldown.
...
The \code{preparePairs} function matches the mapping location of each read to a restriction fragment.
...
<<>>=
diagnostics <- preparePairs("SRR027957.bam", hs.param, file="SRR027957.h5", dedup=TRUE, minq=10)
diagnostics
@
...
The invalid proportion can be used as an empirical measure of the mapping error rate - or, at least, the upper bound of the error rate, given that non-chimeric alignments are longer and more reliable.

<<>>=
diagnostics$invalid/diagnostics$multi
@
...
For full compatibility, users should include the alignment positions and lengths as \code{xxx.pos} and \code{xxx.len} (replacing \code{xxx} with \code{anchor} or \code{target}).
...
It will be used in downstream \pkgname{} functions, such as read counting around blacklisted regions.
...
Recall that each read is assigned to a restriction fragment index.
...
In addition, if \code{restrict} is a 1-by-2 matrix, count loading will be limited to the read pairs that are mapped between the specified pair of chromosomes.
...
We'll also need \code{bin.size} and \code{mm.param}.
...
Here, the threshold is defined as a minimum fold change of 10 above this estimate.

<<>>=
direct.threshold <- median(ave.ab[is.na(dist)], na.rm=TRUE)
direct.threshold
direct.keep <- ave.ab > log2(10) + direct.threshold
...
This is thought to be driven by non-specific compaction of chromatin \cite{lin2012global}.
If such compaction is uninteresting, a concomitantly higher threshold is necessary to offset the increase in counts for these local interactions.

In the trended strategy, a trend is fitted to the average abundance for all intra-chromosomal bin pairs using the log-distance as the covariate. 
...
<<>>=
trend.keep <- ave.ab > trend.threshold
...
Of course, the threshold can also be defined at some minimum fold change above the expected compaction intensity.
The trended filter can also be combined with \code{count.keep} to ensure that the absolute counts are large.
This is particularly important at large distances, where the drop in the filter threshold may lead to the inappropriate retention of very low abundance bins.
...
Peaks in the interaction space refer to bin pairs with substantially more reads than its neighbours.
The \code{enrichedGap} function computes an enrichment value for each bin pair, defined as the log-fold change of its abundance against its neighbours.
Those bin pairs with high enrichment values can then be retained.
The example below uses a smaller bin size to detect potential peaks at higher resolution.
Note that the function requires all bin pairs, i.e., counting in \code{squareCounts} with \code{filter=1}.
Thus, to avoid overloading the machine memory, it may be prudent to analyze each pair of chromosomes separately.

<<>>=
chr1.data <- squareCounts(input, reform(mm.param, restrict="chr1"), width=1e5, filter=1)
enrich.chr1 <- enrichedGap(chr1.data)
summary(enrich.chr1)   
@
...
These regions are designed to capture any high-intensity structural features like TADs, compartments or banding patterns \citep{rao2014kilobase} that might be present.
Thus, any bin pair lying within one of these features will be compared to other high-abundance bin pairs from the same feature.
Otherwise, peaks may be spuriously detected at the feature boundary, where the neighbourhood is diluted by low-abundance bin pairs outside of the feature.

Some adjustment can also be performed to account for distance biases.
A loess curve is fitted to the average abundances against the log-distances, and the fitted value is subtracted from the abundance for each intra-chromosomal bin pair.
This allows a valid comparison between bin pairs at different distances in the interaction space (assuming that the distance effect is uninteresting, e.g., due to compaction).
Of course, this is not required for inter-chromosomal bin pairs, and can be turned off altogether by setting \code{trend="none"}.

Peaks are then defined as those bin pairs with high enrichment values.
A value of 1 is required here, such that the counts for each retained bin pair are at least two-fold higher than those of its neighbourhood.
In practice, filtering of enrichment values should be combined with filtering on absolute abundances.
This eliminates low-abundance bin pairs with high enrichment values, e.g., when the surrounding neighbourhood is empty.
...
Indeed, the definition of ``broad'' depends on the bin size, such that a peak called at large bin sizes may be discarded at lower bin sizes.
Users are advised to try out the other general filters before proceeding to peak calling, to check that potentially interesting differences are not discarded in the latter step.
...
Counts for each chromosome pair can be reloaded for separate filtering, and the remaining bin pairs can be aggregated for downstream analysis.
This is quite slow.
...
The \code{scaledAverage} function is applied to compute the average abundance for each of the small bin pairs. 
This adjusts for the area of the interaction space used for read pair counting, and is necessary for a valid comparison between small and large bin pairs.
For consistency, the abundances of the larger bin pairs are also recomputed.
The resulting values are used to obtain a direct filter threshold, as previously described.
...
Finally, filtering is performed by mandating a minimum fold change for the abundances of the smaller bin pairs over the adjusted threshold (in this case, 10-fold).
...
However, some extra effort is required to match the threshold at each covariate.
...
The computed \code{new.threshold2} can then be used to filter on the abundances of the smaller bin pairs, as described above.
...
Another advantage is that estimation of the threshold is also more precise when the counts are larger.
Thus, even if there is sufficient memory for loading smaller bin pairs with \code{filter=1}, use of larger bin sizes may still be desirable when computing the filter threshold.
Otherwise, calculation of the median or fitting of the trend will be complicated by discreteness for low counts.
...
This means that the filter thresholds may be overestimated, e.g., if the regions were specified such that only high-abundance areas are counted.
Conversely, threshold estimation may be imprecise if the counts for all areas are low.
...
As an aside, adjustment of abundances by area fails to account for other differences between regions, e.g., in mappability or sequenceability.
...
Removal can be achieved by applying the \code{diag.keep} object alongside other filters, as shown below for the direct filter.
...
Short-range interactions will be represented by the remaining off-diagonal bin pairs, due to the improved spatial resolution of smaller bins.
...
This accounts for composition biases by assuming that most interactions are unchanged.
...
In practice, this scaling approach is usually too simplistic for Hi-C.
More sophisticated approaches are necessary to handle the complex biases observed in real data.
...
The example below uses the NB-loess method, which is an adaptation of existing non-linear methods for discrete count data. %\citep{normpaper}. % Coming soon, I hope.
...
For example, wholesale duplication of an entire TAD will double the interaction intensity for intra-TAD interactions (assuming negligble inter-TAD interactions between the original and the copy).
Under factorizability, a 4-fold increase in interaction intensity would be expected as all interacting loci within the TAD are doubled.
...
All covariates are simultaneously used, so that different biases at any combination of covariate values can be accommodated.
...
The safest choice is to avoid it when there is no evidence for CNVs, based on the diagnostic plots in Section~\ref{sec:marginal}.
...
It can also consider extra-Poisson variability between biological replicates, i.e., Hi-C libraries prepared from different biological samples.

The magnitude of biological variability is empirically determined from biological replicates in the experimental design.
This is used during testing to reduce the significance of any detected differences when the data is highly variable.
Estimation of the NB dispersion parameter in \edgeR{} allows modelling of this biological variation.
...
Finally, we need to make sure that the \csaw{} package is loaded, e.g., with \code{library(csaw)}.
\end{combox}
...
The code below shows that many small bin pairs are detected in clusters that represent the underlying interactions.

<<>>=
small.de <- result.small$table$FDR <= 0.05
clustered <- clusterPairs(smaller.data[small.de], tol=1)
tabulate(clustered)[1:10]
@
...
One could obtain larger counts by directly using larger bin pairs, given that spatial resolution is not an issue for diffuse interactions.
...
This method is also a good introduction to the next (more useful) section.
...
Robust results can be obtained by combining the differential analyses from both small and large bin pairs.
...
All of these $p$-values can then be combined.
The idea is to provide a compromise between larger counts and spatial resolution in the final combined $p$-value.
In practice, a weighted version of Simes' method is used to compute the combined $p$-value \citep{benjamini1997multiple}.
...
The code snippet above uses the fact that the count matrix from the \code{DIList} in \code{boxPairs} records the number of nested bin pairs at each bin size.
...
The BH method is also applied to the combined $p$-values.
...
bound1 <- 100
bound3 <- bound1*totals(data)[3]/totals(data)[1]
@
...
Any pixel with a count above \code{max.count} will be set at the maximum colour intensity.
