...
nested <- data.frame(anchor.chr=ax.3$seqnames, anchor.start=ax.3$start,
    anchor.end=ax.3$end, target.chr=tx.3$seqnames, target.start=tx.3$start,
    target.end=tx.3$end, result.small$table[best.vec,])
head(nested)
@
