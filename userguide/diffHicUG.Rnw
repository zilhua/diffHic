...
<<>>=
require(csaw)
smaller.ab <- scaledAverage(asDGEList(smaller.data), 
    scale=(new.bin.size/bin.size)^2)
summary(smaller.ab)
larger.ab <- scaledAverage(asDGEList(data))
...
area <- area/mean(area)
summary(scaledAverage(redata, scale=area))
@
