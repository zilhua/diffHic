...
The identity of the other locus (i.e., from \code{second.regions}) for each interaction can be determined using on the \code{is.second} field in the \code{GRanges} object.
