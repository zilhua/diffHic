...
To consolidate these results, the \code{consolidatePairs} function is used to identify all smaller bin pairs that are nested within each of the larger ``parent'' bin pairs.
Specifically, smaller bin pairs are nested in larger bin pairs with the same ID in \code{cons\$id}.
Note that the larger bin size \textit{must} be an integer multiple of the smaller bin size(s).
This is necessary to simplify the interpretation of the nestprocedure.
...
All of these $p$-values can then be combined into a single $p$-value for the parent.
This is done using a weighted version of Simes' method \citep{benjamini1997multiple}.
