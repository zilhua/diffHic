...
for (chosen in 1:length(a.list)) { 
	...
	plotPlaid(input[3], param=mm.param, width=2.5e4, max.count=capped[chosen], col="black",
			first.region=resize(a.list[chosen], fix="center", width=1e6),
			second.region=resize(t.list[chosen], fix="center", width=1e6),
...
However, this summarization of results is not statistically rigorous.
Controlling the FDR across bin pairs does not guarantee control of the FDR over clusters.
...
<<>>=
csaw::clusterFDR(clustered.sig$id, threshold)
@
...
nest.mid.a <- (final.3$nest.anchor.start[chosen]+final.3$nest.anchor.end[chosen])/2
nest.mid.t <- (final.3$nest.target.start[chosen]+final.3$nest.target.end[chosen])/2
